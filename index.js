    ...
    const run = function (x, config, oa) {
      ...
      const mainInterval = setInterval(function () {
        ...
        if (a) {
          ...
          if (inGame && target.chatSpamEnabled && target.chatSpam) {
            if (!target._lastChatTime || Date.now() - target._lastChatTime > 3000) {
              target._lastChatTime = Date.now();
  ...
  process.on('message', (message) => {
    if (message.type === 'start') {
      ...
      if (message.spam) {
        ...
        target.chatSpamEnabled = true;
      } else {
        target.chatSpam = "";
        target.chatSpamEnabled = false;
        if (message.message) {
          if (currentBotInterface && currentBotInterface.controller) {
